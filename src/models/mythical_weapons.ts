...
export class MythicalWeaponStore{
    ...
    async create(name:string, type:string, weight:number):Promise<MythicalWeapon>{
        try{
            const connection = await client.connect();
            const sql = `insert into mythical_weapons(name, type, weight) values ('${name}', '${type}', ${weight}) RETURNING *`;
            const result = await connection.query(sql);
            await connection.release();
    ...
    async deleteById(id:number):Promise<MythicalWeapon>{
        try{
            const connection = await client.connect();
            const sql = `delete from mythical_weapons where id=${id} RETURNING *`;
            const result = await connection.query(sql);
            await connection.release();
            return result.rows[0];
        }
    ...
    async update(id:number, newType:string, newName:string, newWeight:number):Promise<MythicalWeapon>{
        try{
            const connection = await client.connect();
            const sql = `update mythical_weapons set name = '${newName}', type = '${newType}', weight = ${newWeight} where id = ${id} RETURNING *`;
            const result = await connection.query(sql);
            await connection.release();
            return result.rows[0];
        }
